...
# InternVL
def run_internvl_2b(questions: list[str], modality: str) -> ModelRequestData:
...
def run_internvl_8b(questions: list[str], modality: str) -> ModelRequestData:
...
# InternVL-14B
def run_internvl_14b(questions: list[str], modality: str) -> ModelRequestData:
...
def run_paligemma(questions: list[str], modality: str) -> ModelRequestData:
    ...
    engine_args = EngineArgs(
        ...
        dtype="bfloat16",  # Set consistent dtype to avoid Float/Half mismatch
        enforce_eager=True,  # Disable torch compilation to avoid dtype issues
        limit_mm_per_prompt={modality: 1},
    )
...
model_example_map = {
    ...
    "internvl2b_chat": run_internvl_2b,
    "internvl8b_chat": run_internvl_8b,
    "internvl14b_chat": run_internvl_14b,
    "kimi_vl": run_kimi_vl,
    "llava": run_llava,
    ...
    if args.modality == "image":
        ...
        if args.image_path:
            ...
            try:
                ...
                print(f"Using custom image: {args.image_path}")
            except Exception as e:
                print(f"Error loading image {args.image_path}: {e}")
                print("Falling back to default cherry_blossom image.")
        ...
        if args.question:
            # Use custom question provided by user
            img_questions = [args.question]
            print(f"Using custom question: {args.question}")
        else:
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to custom image file to process. If not provided, uses default cherry_blossom image.",
    )
    parser.add_argument(
        "--question",
        type=str,
        default=None,
        help="Custom question to ask about the image. If not provided, uses default questions.",
    )
