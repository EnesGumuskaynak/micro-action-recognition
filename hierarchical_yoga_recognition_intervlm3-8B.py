...
def parse_args():
    parser = argparse.ArgumentParser(
        description="InternVL3-8B için hiyerarşik yoga pozu sınıflandırma"
    )
...
def main():
    ...
    model_name = path.split("/")[-1]  # InternVL3-8B

    # Tek bir GPU'ya yükleme yapalım
    if torch.cuda.is_available():
        device = torch.device("cuda:0")
        print(f"Model {device} cihazına yükleniyor...")

        # Modeli önce CPU'ya tam olarak yükle
        model = AutoModel.from_pretrained(
            ...
            device_map=None,  # Önce tüm modeli CPU'ya yükle
        )

        # Sonra tüm modeli GPU'ya taşı
